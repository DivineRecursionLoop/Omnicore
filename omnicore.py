...
print(f"[*] Missing dependency detected: ' {
    pip_name
...
print(f"[!] Critical Error: Failed to auto-install {
    pip_name
}. Error: {
    e
}")
...
mod_name = file[:-3]
try:
module = importlib.import_module(f"modules. {
    mod_name
}")
...
logging.info(f"Loaded module script: {
    instance.name
}")
except Exception as e:
print(f" {
...
print(f" {
    ...
    mod_name
}' not found. Type 'list' to see options. {
    Style.RESET_ALL
}")
...
print(f" {
    Fore.RED
}[!] No active module selected. Use 'use <module_name>' first. {
    Style.RESET_ALL
}")
return
...
print(f" {
    Fore.RED
}[!] Unknown argument. Try: show options {
    Style.RESET_ALL
}")
...
print(f"\n {
    Fore.YELLOW
}[OmniCore Workspace Summary] {
    Style.RESET_ALL
}")
print(f"  Tracked Targets : {
    summary['targets']}")
print(f"  Discovered Ports: {
    summary['services']}")
print(f"  Saved Findings  : {
    summary['findings']}\n")
...
print(f"\n {
    Fore.YELLOW
}[Discovered Services in Workspace] {
    Style.RESET_ALL
}")
print(f" {
    'IP Address':<18
...
print(f" {
    Fore.RED
}[!] Unknown workspace command. Try: workspace status or workspace services {
    Style.RESET_ALL
}")
...
print(f" {
    Fore.RED
}[!] No module selected. Use 'use <module_name>' first. {
    Style.RESET_ALL
}")
return
...
print(f" {
    Fore.RED
}[!] Module crashed during execution: {
    e
} {
    Style.RESET_ALL
}")
...
print(f"[*] Loading and executing playbook sequence: {
    Fore.GREEN
} {
    filename
} {
...
print("\n" + "=" * 65)
print(f"[*] Playbook execution completed successfully.")
except Exception as e:
print(f" {
    Fore.RED
